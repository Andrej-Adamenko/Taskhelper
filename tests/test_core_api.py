...
class GetMessagesTest(TestCase):
	def test_messages(self, mock_sleep, mock_info, *args):
		...
		result = core_api.get_messages(channel_id, last_msg_id, limit, time_sleep, client=mock_client)
	...
	def test_messages_with_list(self, mock_sleep, mock_info, *args):
		...
		result = core_api.get_messages(channel_id, last_msg_id, limit, time_sleep, client=mock_client, message_ids=message_ids)
		self.assertEqual(expected_calls, manager.mock_calls)
