...
class SendDailyRemindersTest(TestCase):
	...
	def test_missing_ticket_for_reminding(self, mock_get_message_for_reminding, mock_is_user_reminder_data_exists,
										  mock_get_last_interaction_time, mock_insert_or_update_remind_time, *args):
		user_tag = "aa"
		user_id = 12345678
		main_channel_id = -100123456789
		mock_bot = Mock(spec=TeleBot)
		missing_ticket = [main_channel_id, 123, -100987654321, 33]  # main_message_id, copied_channel_id, copied_message_id
		normal_ticket = [main_channel_id, 198, -100111122222, 11]  # main_message_id, copied_channel_id, copied_message_id
		...
		mock_is_user_reminder_data_exists.assert_called_once_with(user_tag)
		mock_get_last_interaction_time.assert_called_once_with(user_tag)
	...
	def test_no_tickets_for_reminding(self, mock_get_message_for_reminding, mock_is_user_reminder_data_exists,
									  mock_get_last_interaction_time, mock_insert_or_update_remind_time, *args):
		user_tag = "aa"
		user_id = 12345678
		main_channel_id = -100123456789
...
class GetMessageForRemindingTest(TestCase):
	def test_default(self, mock_get_tickets_for_reminding, mock_get_user_individual_channels,
		...
		self.assertEqual(result, (main_channel_id, main_message_id, copied_channel_id, copied_message_id))
